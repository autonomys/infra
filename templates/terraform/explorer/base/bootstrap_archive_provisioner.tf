...
resource "null_resource" "start-archive-nodes" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # create .env file
      "echo NODE_ORG=${var.archive-node-config.node-org} > /home/${var.ssh_user}/archive/.env",
      "echo NODE_TAG=${var.archive-node-config.node-tag} >> /home/${var.ssh_user}/archive/.env",
