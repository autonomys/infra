...
module "eks" {
  ...
  #we uses only 1 security group to allow connection with Karpenter nodes
  ...
  aws_auth_roles = concat(
    [for team in module.eks_dev_teams : team.aws_auth_configmap_role],
    [
      module.eks_platform_teams.aws_auth_configmap_role,
      {
        rolearn  = module.eks_addons.karpenter.node_iam_role_arn
...
module "eks_dev_teams" {
  source  = "aws-ia/eks-blueprints-teams/aws"
  version = "~> 1.0"


  for_each = var.team_labels
  name     = "team-${each.key}"
