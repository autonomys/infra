...
resource "aws_instance" "evm_node" {
  count              = length(var.aws_region) * var.evm-node-config.instance-count
  ami                = data.aws_ami.ubuntu_amd64.image_id
  instance_type      = var.evm-node-config.instance-type
  subnet_id          = element(aws_subnet.public_subnets.*.id, 0)
  availability_zone  = var.azs
  ipv6_address_count = 1
  # Security Group
  ...
  ebs_block_device {
    device_name = "/dev/sda1"
    volume_size = var.evm-node-config.disk-volume-size
    volume_type = var.evm-node-config.disk-volume-type
    iops        = 3000
    throughput  = 250
  }
