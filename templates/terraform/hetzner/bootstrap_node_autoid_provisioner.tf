locals {
  bootstrap_nodes_autoid_ip_v4 = flatten([
    [var.bootstrap-node-autoid-config.additional-node-ips]
    ]
  )
}
...
resource "null_resource" "start-bootstrap-nodes-autoid" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # create .env file
      "echo NODE_ORG=${var.bootstrap-node-autoid-config.repo-org} > /root/subspace/.env",
      "echo NODE_TAG=${var.bootstrap-node-autoid-config.node-tag} >> /root/subspace/.env",
      ...
      "echo DSN_LISTEN_PORT=${var.bootstrap-node-autoid-config.dsn-listen-port} >> /root/subspace/.env",
      "echo NODE_DSN_PORT=${var.bootstrap-node-autoid-config.node-dsn-port} >> /root/subspace/.env",
      "echo OPERATOR_PORT=${var.bootstrap-node-autoid-config.operator-port} >> /root/subspace/.env",
      "echo GENESIS_HASH=${var.bootstrap-node-autoid-config.genesis-hash} >> /root/subspace/.env",
      "echo BRANCH_NAME=${var.branch_name} >> /root/subspace/.env",

      # create docker compose file
      "bash /root/subspace/create_compose_file.sh ${var.bootstrap-node-autoid-config.reserved-only} ${length(local.bootstrap_nodes_autoid_ip_v4)} ${count.index} ${length(local.bootstrap_nodes_ip_v4)} ${var.domain-node-config.enable-domains}",
