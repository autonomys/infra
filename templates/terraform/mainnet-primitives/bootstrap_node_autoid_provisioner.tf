...
resource "null_resource" "start-bootstrap-nodes-autoid" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo DOMAIN_LABEL_AUTO=${var.evm-node-config.domain-labels[1]} >> /home/${var.ssh_user}/subspace/.env",
      "echo DOMAIN_ID_AUTO=${var.evm-node-config.domain-id[1]} >> /home/${var.ssh_user}/subspace/.env",
