...
// subspace Grafana monitor
resource "cloudflare_record" "grafana" {
  comment = "Grafana and Loki monitoring"
  name    = "grafana"
  proxied = true
  ttl     = 1
  type    = "A"
  value   = "138.201.195.122"
  zone_id = data.cloudflare_zone.subspace_network.id
}

// subspace Loki logs
resource "cloudflare_record" "logging" {
  comment = "Loki logging server"
  name    = "grafana"
  proxied = false
  ttl     = 3600
  type    = "A"
...
// subspace victoria metrics monitoring (temporary)
resource "cloudflare_record" "vmetrics" {
  comment = "Victoria Metrics monitoring"
  name    = "vmetrics"
  proxied = false
  ttl     = 3600
  type    = "A"
