...
output "bare_domain_bootstrap_node_public_ip" {
  value = var.bare-domain-bootstrap-node-config == null ? null : var.bare-domain-bootstrap-node-config.bootstrap-nodes.*.ipv4
}
...
output "timekeeper_node_public_ip" {
  value = var.timekeeper-node-config == null ? null : var.timekeeper-node-config.timekeeper-nodes.*.ipv4
}
...
output "dns-records" {
  value = concat(
    ...
    [for record in cloudflare_dns_record.bare_domain_bootstrap_ipv4 :
      format("%s.%s", trimsuffix(record.name, ".${data.cloudflare_zone.cloudflare_zone.name}"), data.cloudflare_zone.cloudflare_zone.name)
    ],
