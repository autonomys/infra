...
resource "null_resource" "start-nova-blue-squid-nodes" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "sudo certbot --nginx --non-interactive -v --agree-tos -m alerts@subspace.network -d squid.${var.network_name}.subspace.network -d ${var.nova-blue-squid-node-config.domain-prefix}.${var.network_name}.subspace.network",
      ...
      "echo DOMAIN_PREFIX=${var.nova-blue-squid-node-config.domain-prefix} >> /home/${var.ssh_user}/squid/.env",
      "echo NR_API_KEY=${var.nr_api_key} >> /home/${var.ssh_user}/squid/.env",
      "echo DOCKER_TAG=${var.nova-blue-squid-node-config.docker-tag} >> /home/${var.ssh_user}/squid/.env",
      "echo NODE_NAME=SUBSQUID_GEMINI_3g >> /home/${var.ssh_user}/squid/.env",
      ...
      "echo ARCHIVE_ENDPOINT=https://archive.gemini-3g.subspace.network/api >> /home/${var.ssh_user}/squid/.env",
      "echo CHAIN_RPC_ENDPOINT=wss://rpc-0.gemini-3g.subspace.network/ws >> /home/${var.ssh_user}/squid/.env",
...
resource "null_resource" "start-nova-green-squid-nodes" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "echo NR_API_KEY=${var.nr_api_key} >> /home/${var.ssh_user}/squid/.env",
      "echo DOCKER_TAG=${var.nova-green-squid-node-config.docker-tag} >> /home/${var.ssh_user}/squid/.env",
      "echo NODE_NAME=SUBSQUID_GEMINI_3g >> /home/${var.ssh_user}/squid/.env",
      "echo POSTGRES_HOST=db >> /home/${var.ssh_user}/squid/.env",
      "echo POSTGRES_PORT=5432 >> /home/${var.ssh_user}/squid/.env",
      "echo POSTGRES_USER=postgres >> /home/${var.ssh_user}/squid/.env",
      ...
      "echo CHAIN_RPC_ENDPOINT=wss://nova.${var.network_name}.subspace.network/ws >> /home/${var.ssh_user}/squid/.env",
      "echo PROCESSOR_HEALTH_HOST=http://processor:3000 >> /home/${var.ssh_user}/squid/.env",
      "echo PROCESSOR_HEALTH_PORT=7070 >> /home/${var.ssh_user}/squid/.env",
      "echo HEALTH_CHECK_PORT=8080 >> /home/${var.ssh_user}/squid/.env",
      "echo INGEST_HEALTH_HOST=http://ingest:9090 >> /home/${var.ssh_user}/squid/.env",
