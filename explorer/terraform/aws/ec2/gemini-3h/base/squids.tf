...
resource "aws_instance" "nova_squid_blue_node" {
  ...
  ebs_block_device {
    device_name = "/dev/sda1"
    volume_size = var.nova-blue-squid-node-config.disk-volume-size
    volume_type = var.nova-blue-squid-node-config.disk-volume-type
    iops        = 3000
    throughput  = 250
  }
...
resource "aws_instance" "nova_squid_green_node" {
  ...
  ebs_block_device {
    device_name = "/dev/sda1"
    volume_size = var.nova-green-squid-node-config.disk-volume-size
    volume_type = var.nova-green-squid-node-config.disk-volume-type
    iops        = 3000
    throughput  = 250
  }
