resource "digitalocean_droplet" "archive-squid-nodes" {
  ...
  name      = "${var.archive-squid-node-config.network-name}-archive-node-${count.index}-${var.archive-squid-node-config.regions[count.index % length(var.archive-squid-node-config.regions)]}"
...
resource "digitalocean_volume" "archive-squid-nodes-additional-volume" {
  count                   = length(var.archive-squid-node-config.regions) * var.archive-squid-node-config.nodes-per-region
  region                  = var.archive-squid-node-config.regions[count.index % length(var.archive-squid-node-config.regions)]
  name                    = "${var.archive-squid-node-config.network-name}-archive-squid-node-${count.index}-${var.archive-squid-node-config.regions[count.index % length(var.archive-squid-node-config.regions)]}-vol"
...
resource "digitalocean_droplet" "explorer-nodes" {
  ...
  image     = "ubuntu-22-10-x64"
  name      = "${var.archive-squid-node-config.network-name}-explorer-node-${count.index}-${var.explorer-node-config.regions[count.index % length(var.explorer-node-config.regions)]}"
...
resource "digitalocean_volume" "explorer-nodes-additional-volume" {
  count                   = length(var.explorer-node-config.regions) * var.explorer-node-config.nodes-per-region
  region                  = var.explorer-node-config.regions[count.index % length(var.explorer-node-config.regions)]
  name                    = "${var.archive-squid-node-config.network-name}-explorer-node-${count.index}-${var.explorer-node-config.regions[count.index % length(var.explorer-node-config.regions)]}-vol"
