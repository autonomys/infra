...
resource "aws_instance" "linux_x86_64_runner" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      # configure runner
      "./config.sh --url https://github.com/subspace --token ${var.gh_token} --unattended --name ubuntu-22.04-x86-64 --labels 'self-hosted,self-hosted-ubuntu-22.04-x86-64,ubuntu-22.04-x86-64,Linux,x86-64' --work _work --runasservice",
...
#       # configure runner
#       "./config.sh --url https://github.com/subspace --token ${var.gh_token} --unattended --name ubuntu-22.04-arm64 --labels 'self-hosted,self-hosted-ubuntu-22.04-arm64,ubuntu-22.04-arm64,Linux,arm64' --work _work --runasservice",
#       "sudo ./svc.sh install ${var.ssh_user[0]},
#       "sudo ./svc.sh start",
#       # install monitoring
#       "sudo sh -c \"curl https://my-netdata.io/kickstart.sh > /tmp/netdata-kickstart.sh && sh /tmp/netdata-kickstart.sh --non-interactive --nightly-channel --claim-rooms ${var.netdata_room} --claim-token ${var.netdata_token} --claim-url https://app.netdata.cloud\"",
...
#       "./config.sh --url https://github.com/subspace --token ${var.gh_token} --unattended --name macos-12-x86-64 --labels 'self-hosted,self-hosted-macos-12-x86-64,macos-12-x86-64,macOS,x86-64' --work _work --runasservice",
...
resource "aws_instance" "mac_arm64_runner" {
  ...
  provisioner "remote-exec" {
    inline = [
      ...
      "./config.sh --url https://github.com/subspace --token ${var.gh_token} --unattended --name macos-12-arm64 --labels 'self-hosted,self-hosted-macos-12-arm64,macos-12-arm64,macOS,arm64' --work _work --runasservice",
      "sudo su -- ${var.ssh_user[1]} ./svc.sh install",
      "sudo su -- ${var.ssh_user[1]} ./runsvc.sh start &",
      # install monitoring
      "NONINTERACTIVE=1 /bin/bash -c \"$(curl -fsSL https://raw.githubusercontent.com/Homebrew/install/master/install.sh)\"",
...
#       # configure runner
#       "./config.cmd --url https://github.com/subspace --token ${var.gh_token} --unattended --name windows-server-2022-x86-64 --labels 'self-hosted,self-hosted-windows-server-2022-x86-64,windows-server-2022-x86-64,Windows,x86-64' --work _work --runasservice",
#     ]
