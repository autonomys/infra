...
resource "digitalocean_volume" "aries-relaynet-test" {
  region                  = "nyc3"
  name                    = "relaynet-test-volume"
  size                    = 1800
  initial_filesystem_type = "ext4"
  description             = "Extra volume for relaynet a."
}

resource "digitalocean_volume_attachment" "aries-relaynet-test" {
  droplet_id = digitalocean_droplet.aries-relaynet-test.id
  volume_id  = digitalocean_volume.aries-relaynet-test.id
}
