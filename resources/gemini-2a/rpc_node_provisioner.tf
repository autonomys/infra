...
#  triggers = {
#    cluster_instance_ipv4s = join(",", digitalocean_droplet.gemini-2a-full-nodes.*.ipv4_address)
#  }
#
#  # generate rpc node keys
#  provisioner "local-exec" {
#    command = "../../scripts/generate_node_keys.sh ${length(digitalocean_droplet.gemini-2a-full-nodes)} ./rpc_node_keys.txt"
#    interpreter = [ "/bin/bash", "-c" ]
#    environment = {
#      NODE_PUBLIC_IPS = join(",", digitalocean_droplet.gemini-2a-full-nodes.*.ipv4_address)
#    }
#  }
#}
#
#resource "null_resource" "setup-rpc-nodes" {
#  count = length(digitalocean_droplet.gemini-2a-full-nodes)
#
...
#
#  connection {
#    host = digitalocean_droplet.gemini-2a-full-nodes[count.index].ipv4_address
#    user = "root"
#    type = "ssh"
#    agent = true
...
#      "sudo /subspace/create_compose_file.sh ${length(digitalocean_droplet.gemini-2a-full-nodes)} ${count.index} ${length(digitalocean_droplet.gemini-2a-bootstrap-nodes)}",
