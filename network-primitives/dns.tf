...
resource "cloudflare_record" "core-domain-rpc" {
  count   = length(local.rpc_node_ip_v4)
  zone_id = data.cloudflare_zone.cloudflare_zone.id
  name    = "${var.rpc-node-config.domain-prefix}-${count.index}.${var.domain-node-config.domain-labels[count.index]}.${var.network-name}"
  value   = local.rpc_node_ip_v4[count.index]
  type    = "A"
}
